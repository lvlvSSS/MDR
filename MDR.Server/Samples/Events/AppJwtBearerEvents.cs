...
{
    private readonly JwtTokenParameterOptions _jwtOptions;
    private ILogger infoLogger;
    private ILogger errorLogger;

    public AppJwtBearerEvents(IOptionsSnapshot<JwtTokenParameterOptions> jwtOptions, ILoggerFactory loggerFactory)
    {
        _jwtOptions = jwtOptions.Value;
        infoLogger = loggerFactory.CreateLogger("info");
        errorLogger = loggerFactory.CreateLogger("error");
    }
    ...
    public override Task MessageReceived(MessageReceivedContext context)
    {
        ...
        {
            errorLogger.LogError("no jwt authorization code.");
            context.NoResult();
            return Task.CompletedTask;
        }
