...
public static class PinYinExtension
{
    ...
    /// 获取单字拼音
    /// </summary>
    /// <param name="ch">中文汉字</param>
    /// <returns>返回拼音字符串</returns>
    public static string PinYin(this char ch)
    {
        // 拉丁字符            
    ...
    /// 取拼音第一个字段
    /// </summary>        
    /// <param name="ch">中文字符</param>        
    /// <returns>中文字符的第一个拼音</returns>    
    public static string FirstPinYin(this char ch)
    {
        string pinyin = ch.PinYin();
    ...
    /// 取拼音第一个字段
    /// </summary>
    /// <param name="src">中文字符字符串</param>
    /// <returns>返回每个中文的第一个拼音字符</returns>
    public static string FirstPinYin(this string src)
    {
        if (string.IsNullOrWhiteSpace(src)) return string.Empty;

        var sb = new StringBuilder(src.Length + 1);
        var chs = src.ToCharArray();

        for (var i = 0; i < chs.Length; i++)
        {
            sb.Append(chs[i].FirstPinYin());
        }
    ...
    /// 把汉字转换成拼音(全拼)
    /// </summary>
    /// <param name="src">汉字字符串</param>
    /// <returns>转换后的拼音(全拼)字符串</returns>
    public static string PinYin(this string src)
    {
        if (string.IsNullOrWhiteSpace(src)) return string.Empty;

        var sb = new StringBuilder(src.Length * 10);
        var chs = src.ToCharArray();

        for (var j = 0; j < chs.Length; j++)
        {
            sb.Append(chs[j].PinYin());
        }
